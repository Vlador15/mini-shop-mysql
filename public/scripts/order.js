$(document).ready(function () {
  $('input[type="tel"]').inputmask({ mask: "89999999999" });
  ...
  $(".js-form-validate").submit(function () {
    ...
    if (data["name"].value.length < 5 || data["name"].value.length > 50)
      err(data["name"]);
    if (data["address"].value.length < 3 || data["address"].value.length > 70)
      err(data["address"]);
    if (data["phone"].value.length != 12) err(data["phone"]);

    if (data["city"].value.length < 2 || data["city"].value.length > 50)
      err(data["city"]);
